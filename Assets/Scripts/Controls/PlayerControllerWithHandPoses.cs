...
public class PlayerControllerWithHandPoses : MonoBehaviour
{
	/// <summary>
	...
	public void SetHandPoseMoveForward(bool state)
	{
		...
		Debug.Log($"SetHandPoseMoveForward state changed to: {state}");
	}
	public void SetHandPoseMoveBackward(bool state)
	{
		...
		Debug.Log($"SetHandPoseMoveBackward state changed to: {state}");
	}
	public void SetHandPoseRotateRight(bool state)
	{
		HandPoseRotateRight = state;
		Debug.Log($"SetHandPoseRotateRight state changed to: {state}");
	}
	public void SetHandPoseRotateLeft(bool state)
	{
		HandPoseRotateLeft = state;
		Debug.Log($"SetHandPoseRotateLeft state changed to: {state}");
	}
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
			...
			if (moveBack)
				MoveThrottle += ort * (transform.lossyScale.z * moveInfluence * BackAndSideDampen * (Vector3.back + Vector3.left));
