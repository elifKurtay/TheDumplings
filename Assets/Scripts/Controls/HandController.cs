...
public class HandController : MonoBehaviour {
	...
	static protected ObjectAnchor[] anchors_in_scene;
	void Start () {
		// Prevent multiple fetch
		if ( anchors_in_scene == null ) anchors_in_scene = GameObject.FindObjectsOfType<ObjectAnchor>();
	}
	...
	protected void handle_controller_behavior () {
		...
		if ( hand_closed ) {
			...
			// Get the closest object (with an anchor) to the hand
			int best_object_id = getClosestObject();
			
			// If the best object is in range grab it
			if ( best_object_id != -1 ) {

				// Store in memory the object grasped
				object_grasped = anchors_in_scene[best_object_id];
	...
	/// <summary>
	/// Return the closest object (with an anchor) to the hand
	/// </summary>
	private int getClosestObject() {	
		...
		for ( int i = 0; i < anchors_in_scene.Length; i++ ) {

			// Skip object not available
			if ( !anchors_in_scene[i].is_available() ) continue;

			// Compute the distance to the object
			oject_distance = Vector3.Distance( this.transform.position, anchors_in_scene[i].transform.position );

			// Keep in memory the closest object
			// N.B. We can extend this selection using priorities
			if ( oject_distance < best_object_distance && oject_distance <= anchors_in_scene[i].get_grasping_radius() ) {
				best_object_id = i;
